...
struct node
{
  ...
  typedef
      typename boost::pointer_traits<VoidPtr>::template rebind_to<node>::type
          node_pointer;
...
struct bucket
{
  ...
  typedef
      typename boost::pointer_traits<VoidPtr>::template rebind_to<bucket>::type
          bucket_pointer;
...
struct bucket_group
{
  ...
  typedef typename boost::pointer_traits<bucket_pointer>::template rebind_to<
...
public:
  ...
  pointer operator->() const BOOST_NOEXCEPT { return boost::to_address(p); }
  ...
  bool operator==(grouped_bucket_iterator const& other) const BOOST_NOEXCEPT
  {
    return equal(other);
  }
...
private:
  ...
  void increment() BOOST_NOEXCEPT
  {
    ...
    std::size_t n = std::size_t(
        boost::core::countr_zero(pbg->bitmask & reset_first_bits(offset + 1)));
...
public:
  ...
  bool operator==(grouped_local_bucket_iterator const& other) const
      BOOST_NOEXCEPT
  {
    return equal(other);
  }
...
public:
  ...
  bool operator==(const_grouped_local_bucket_iterator const& other) const
      BOOST_NOEXCEPT
  {
    return equal(other);
  }
...
private:
  ...
  bool equal(const const_grouped_local_bucket_iterator& x) const BOOST_NOEXCEPT
  {
    return p == x.p;
  }
...
public:
  typedef typename boost::allocator_rebind<
      Allocator,
...
public:
  ...
  grouped_bucket_array(BOOST_RV_REF(grouped_bucket_array) other) BOOST_NOEXCEPT
      : empty_value<node_allocator_type>(empty_init_t(),
  ...
  grouped_bucket_array& operator=(BOOST_RV_REF(grouped_bucket_array)
  ...
  void deallocate() BOOST_NOEXCEPT
  {
    if (buckets) {
      bucket_allocator_type bucket_alloc = this->get_bucket_allocator();
      boost::allocator_deallocate(bucket_alloc, buckets, this->buckets_len());
    ...
    if (groups) {
      group_allocator_type group_alloc = this->get_group_allocator();
      boost::allocator_deallocate(group_alloc, groups, this->groups_len());
  ...
  size_type groups_len() const BOOST_NOEXCEPT { return size_ / group::N + 1; }
  ...
  void insert_node_hint(iterator itb,
                        node_pointer p,
                        node_pointer hint) BOOST_NOEXCEPT
  {
    this->append_bucket_group(itb);
