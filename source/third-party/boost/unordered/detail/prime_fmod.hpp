...
struct prime_fmod_size
{
  ...
  static inline boost::uint64_t get_remainder(boost::uint64_t fractional,
                                              boost::uint32_t d)
  {
#  if defined(_MSC_VER)
...
#  elif defined(BOOST_HAS_INT128)
    return static_cast<boost::uint64_t>(((boost::uint128_type)fractional * d)
                                        >> 64);
#  else
...
#  endif /* defined(_MSC_VER) */
  ...
  static inline boost::uint32_t fast_modulo(boost::uint32_t a,
                                            boost::uint64_t M,
                                            boost::uint32_t d)
  {
  ...
  static inline std::size_t position(std::size_t hash, std::size_t size_index)
  {
    ...
    if (BOOST_LIKELY(size_index < sizes_under_32bit)) {
      return fast_modulo(boost::uint32_t(hash) + boost::uint32_t(hash >> 32),
...
template<class T>
std::size_t const prime_fmod_size<T>::sizes_len =
    BOOST_PP_SEQ_SIZE(BOOST_UNORDERED_PRIME_FMOD_SIZES);
...
template<class T>
std::size_t const prime_fmod_size<T>::inv_sizes32_len = sizeof(inv_sizes32)
